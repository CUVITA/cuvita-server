...
router.get('/lists/:realm/:category/:region', [ validator.query('skip').exists().toInt() ],async (req, res) => {
  ...
  let { geoLocation: { lat, long } } = await database.findOne('regions', { region });
  return res.json(await database.find('vendors', { realm, category, location: { $geoWithin: { $centerSphere: [ [ long, lat ], geoWithinRadius ] } }, name: keyword ? new RegExp(keyword, 'ig') : new RegExp('.') }, { projection: { "_id": 0, "category": 0, "address": 0, "location": 0 }, sort: { "rating": -1 }, limit: 10, skip }));
});
